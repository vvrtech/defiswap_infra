  ...
  return (
    <Wrapper maxHeight={maxHeight}>
      <picture>
        ...
        <source type="image/gif" srcSet={getSrcSet(path, attributes[0].src, '.gif')} />
      ...
      {attributes.map((image) => (
        <ImageWrapper key={image.src}>
          <picture>
            ...
            <source type="image/gif" srcSet={getSrcSet(path, image.src, '.gif')} />
            {/* <source type="application/json" srcSet={getSrcSet(path, image.src, '.json')} /> */}
            <img src={getImageUrl(path, image.src)} alt={image.alt} loading="lazy" decoding="async" />
            {/* <Lottie loop={true} animationData={getImageUrl(path, image.src)} alt={image.alt}/> */}
          </picture>
        </ImageWrapper>
      ))}
    </Wrapper>
