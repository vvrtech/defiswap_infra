...
const TotalToken = ({ total, token }: { total: BigNumber; token: Token }) => {
  // let suffix = {token.symbol==='CAKE'?'CAKE':token.symbol}
  if (total && total.gte(0)) {
    return <Balance small value={getBalanceNumber(total, token.decimals)} decimals={0} unit={` ${token.symbol === 'CAKE' ? 'CAKE' : token.symbol}`} />
  }
