...
const MaxTokenEntry = ({ maxToken, ifo, poolId }: { maxToken: number; ifo: Ifo; poolId: PoolIds }) => {
  ...
  const basicTooltipContent =
    ifo.version >= 3.1
      ? t(
          'For the private sale, each eligible participant will be able to commit any amount ofDEFC up to the maximum commit limit, which is published along with the IFO voting proposal.',
        )
      : t(
          'For the basic sale, MaxDEFC entry is capped by minimum between your averageDEFC balance in the iCAKE, or the pool’s hard cap. To increase the max entry, Stake moreDEFC into the iCAKE',
        )

  const unlimitedToolipContent =
    ifo.version >= 3.1 ? (
      <Box>
        <Text display="inline">{t('For the public sale, MaxDEFC entry is capped by')} </Text>
        <Text bold display="inline">
          {t('the number of iCAKE.')}{' '}
        </Text>
        <Text display="inline">
          {t('Lock moreDEFC for longer durations to increase the maximum number ofDEFC you can commit to the sale.')}
        </Text>
      </Box>
    ) : (
      t(
        'For the unlimited sale, MaxDEFC entry is capped by your averageDEFC balance in the iCake. To increase the max entry, Stake moreDEFC into the iCake',
      )
    )
