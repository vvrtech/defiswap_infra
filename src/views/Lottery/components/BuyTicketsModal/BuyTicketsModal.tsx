...
const BuyTicketsModal: React.FC<BuyTicketsModalProps> = ({ onDismiss }) => {
  ...
  const { balance: userCake, fetchStatus } = useTokenBalance(tokens.defc.address)
  ...
  useEffect(() => {
    const getMaxPossiblePurchase = () => {
      ...
      // If the users' maxDEFC balance purchase is less than the contract limit - factor the discount logic into the max number of tickets they can purchase
  ...
  const getErrorMessage = () => {
    if (userNotEnoughCake) return t('InsufficientDEFC balance')
