...
const PredictionConfigProviders = ({ children }) => {
  ...
  const [selectedToken, setConfig] = useState(() => {
    ...
    if ([PredictionSupportedSymbol.BNB, PredictionSupportedSymbol.DEFC].includes(initToken)) {
      return initToken
    }

    return PredictionSupportedSymbol.DEFC
  })

  useEffect(() => {
    const upperToken = _toUpper(token as string) as PredictionSupportedSymbol

    if ([PredictionSupportedSymbol.BNB, PredictionSupportedSymbol.DEFC].includes(upperToken)) {
