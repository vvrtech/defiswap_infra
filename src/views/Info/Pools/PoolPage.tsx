        ...
        <>
          <Flex justifyContent="space-between" mb="16px" flexDirection={['column', 'column', 'row']}>
            <Breadcrumbs mb="32px">
              ...
              <Flex>
                <Text mr="8px"> {`${poolData.token0.symbol === 'Cake'?'DEFC':poolData.token0.symbol} / ${poolData.token1.symbol}`}</Text>
              </Flex>
            </Breadcrumbs>
          ...
          <Flex flexDirection="column">
            <Flex alignItems="center" mb={['8px', null]}>
              ...
              >
                {`${poolData.token0.symbol === 'Cake'?'DEFC':poolData.token0.symbol} / ${poolData.token1.symbol}`}</Text>
            </Flex>
              ...
              <Flex flexDirection={['column', 'column', 'row']} mb={['8px', '8px', null]}>
                <NextLinkFromReactRouter to={`/info/token/${poolData.token0.address}`}>
                  <TokenButton>
                    ...
                    <Text fontSize="16px" ml="4px" style={{ whiteSpace: 'nowrap' }} width="fit-content">
                      {`1 ${poolData.token0.symbol === 'Cake'?'DEFC':poolData.token0.symbol} =  ${formatAmount(poolData.token1Price, {
                        notation: 'standard',
                        displayThreshold: 0.001,
                        tokenPrecision: true,
                      })} ${poolData.token1.symbol === 'Cake'?'DEFC':poolData.token1.symbol}`}
                    </Text>
                  </TokenButton>
                </NextLinkFromReactRouter>
                <NextLinkFromReactRouter to={`/info/token/${poolData.token1.address}`}>
                  <TokenButton ml={[null, null, '10px']}>
                    ...
                    <Text fontSize="16px" ml="4px" style={{ whiteSpace: 'nowrap' }} width="fit-content">
                      {`1 ${poolData.token1.symbol === 'Cake'?'DEFC':poolData.token1.symbol} =  ${formatAmount(poolData.token0Price, {
                        notation: 'standard',
                        displayThreshold: 0.001,
                        tokenPrecision: true,
                      })} ${poolData.token0.symbol === 'Cake'?'DEFC':poolData.token0.symbol}`}
                    </Text>
                  </TokenButton>
                </NextLinkFromReactRouter>
              </Flex>
