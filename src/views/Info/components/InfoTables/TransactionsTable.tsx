      ...
      <LinkExternal href={getBscScanLink(transaction.hash, 'transaction')}>
        <Text>
          {transaction.type === TransactionType.MINT
            ? t('Add %token0% and %token1%', { token0: transaction.token0Symbol==='Cake'?"DEFC":transaction.token0Symbol, token1: transaction.token1Symbol==='Cake'?"DEFC":transaction.token1Symbol })
            : transaction.type === TransactionType.SWAP
            ? t('Swap %token0% for %token1%', { token0: inputTokenSymbol==='Cake'?"DEFC":inputTokenSymbol, token1: outputTokenSymbol==='Cake'?"DEFC":outputTokenSymbol })
            : t('Remove %token0% and %token1%', { token0: transaction.token0Symbol==='Cake'?"DEFC":transaction.token0Symbol, token1: transaction.token1Symbol==='Cake'?"DEFC":transaction.token1Symbol })}
        </Text>
      </LinkExternal>
      <Text>${formatAmount(transaction.amountUSD)}</Text>
      <Text>
        <Text>{`${formatAmount(abs0)} ${transaction.token0Symbol==='Cake'?"DEFC":transaction.token0Symbol}`}</Text>
      </Text>
      <Text>
        <Text>{`${formatAmount(abs1)} ${transaction.token1Symbol==='Cake'?"DEFC":transaction.token1Symbol}`}</Text>
      </Text>
